...
ks = klusim.KluSIM(n_clusters, metric='euclidean', access_method_tree='KDTree', ref_point = 'mean')
...
medoids = kmedoids_initialization.InitializeMedoids(X, n_clusters, heuristic='BUILD', metric='euclidean')
ks = klusim.KluSIM(n_clusters, metric='euclidean', access_method_tree='KDTree', ref_point = 'median')
ks.set_medoids_idx(medoids)
ks_results = ks.swap(X)
